...
public static class AircraftGizmos
{
    ...
    public static void AircraftPhysicsGizmos(Physics phys, GizmoType gizmoType)
    {
        AircraftPhysicsDisplaySettings settings = AircraftPhysicsDisplaySettings.Instance;
        Vector3 weight = phys.GetComponent<Rigidbody>().mass * UnityEngine.Physics.gravity;
    ...
    public static void SurfaceGizmos(Surface surf, GizmoType gizmoType)
    {
        ...
        if (settings.showForces)
        {
            ...
            if (settings.scaleForcesByWeight)
            {
                scale /= rb.mass * UnityEngine.Physics.gravity.magnitude;
            }
            DrawForces(surf.transform, surf.CurrentLift * scale, surf.CurrentDrag * scale, surf.CurrentTorque * scale);
        }
    }

    private static void DrawSurface(Transform transform, SurfaceConfig config, float flapAngle, bool isAtStall = false)
    {
        AircraftPhysicsDisplaySettings settings = AircraftPhysicsDisplaySettings.Instance;
